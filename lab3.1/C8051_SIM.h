...
// Variable to lock out Sim_Update to prevent recursion
uint8_t _sim_update_active = 0;
...
void Sim_Update(){
    // Make sure we aren't calling this recursively (inside an interrupt...)
    if(_sim_update_active) return;
    _sim_update_active = 1;
